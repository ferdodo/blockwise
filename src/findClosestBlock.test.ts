...
import { findClosestBlock, UNIT_BLOCK } from "blockwise";
import { test, expect } from "vitest";

test("Should find the closest block", () => {
...
test("Passing an empty block list should throw an error", () => {
	expect(() => {
		findClosestBlock([], UNIT_BLOCK);
	}).toThrowError("Block list should not be empty !");
});

test("Should return reference to the first block, when two are having the same closest distance", () => {
	const blockList: Block[] = [
		{ x: 1, y: 1, w: 1, h: 1 },
		{ x: 1, y: 1, w: 1, h: 1 },
		{ x: 10, y: 0, w: 1, h: 1 },
